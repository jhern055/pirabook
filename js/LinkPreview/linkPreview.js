...
(function ($) {
    $.fn.linkPreview = function (options) {
        ...
        var defaultTitle = "Sin titulo";
        var defaultDescription = "Sin description";
        ...
        var crawlText = function () {
            ...
            if (trim(text) !== "") {
                ...
                if (block === false && urlRegex.test(text)) {
                    ...
                    $('#previewButtons_' + selector).hide();
                    $('#previewLoading_' + selector).html("<img src='../css/LinkPreview/img/loader.gif' />");
                    $('#photoNumber_' + selector).val(0);

                    allowPosting = false;
                    isCrawling = true;

                    $.post('../linkpreviewcontrol/textCrawler', {
                        text: text,
        ...
        $("div.favoriteRow > div#submit").click(function () {

        // $('#postPreviewButton_' + selector).click(function () {
var element=$(this).parent().find('div.favoriteDiv').get(0);
   var parentid=   $(element).find('select#favorite').val()
    ;
    ...
    {parentid=$(element).data('parentid'); }
            ...
            title = $("span#previewSpanTitle_"+selector).html();
            description = $('#previewDescription_' + selector).html();

            if (((trim(text) !== "") || (trim(text) === "" && trim(hrefUrl) !== "")) && (allowPosting === true && isCrawling === false)) {
                $.get('../linkpreviewcontrol/highlightUrls', {
                    text: text,
                    ...
                    if (title.indexOf(defaultTitle) != -2) {
                    ...
                    // console.log("texto:"+ $('#text_' + selector).val()
