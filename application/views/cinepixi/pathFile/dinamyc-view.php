<?php $sessioMode=$this->session->userdata("sessionMode_pathFile");?>
    ...
    buttons: {
        Si: function() {
			...
			$.ajax({
				    ...
				    url: url+"cinepixi/pathFile/pathFile_delete",
				    type: 'POST',
				    dataType: 'json',
				    data: {
				    	id:id
				    ...
				    success: function(response){
							...
							modal: true,
							    buttons: {
							    	...
							    	if(response.status)
            						{window.location.href="<?php echo base_url().'cinepixi/pathFile/'; ?>";}
