<?php $sessioMode=$this->session->userdata("oessionMode_OpeningBalance");?>
...
form.submit=function(mode,item){
	...
	// od_OpeningBalance :"<?php  echo (isset($id)?$id:'');?>",
    ...
    buttons: {
        Si: function() {
			...
			$.ajax({
				    ...
				    url: url+"admin/sale/openingBalance/openingBalance_delete",
				    type: 'POST',
				    dataType: 'json',
				    data: {
				    	id:id
				    ...
				    success: function(response){
							...
							modal: true,
							    buttons: {
							        ...
							        if(response.status)
            						{window.location.href="<?php echo base_url().'admin/sale/openingBalance/'; ?>";}
 ...
 obj.url="<?php echo base_url();?>";
	...
	$.ajax({
        ...
        data:{
        	...
        	module:"<?php echo encode_id('opening_balance'); ?>",
