<?php $sessioMode=$this->session->userdata("sessionMode_sale");?>
    ...
    buttons: {
        Si: function() {
			...
			$.ajax({
				    ...
				    url: url+"admin/sale/sale_delete",
				    type: 'POST',
				    dataType: 'json',
				    data: {
				    	id:id
				    ...
				    success: function(response){
							...
							modal: true,
							    buttons: {
							        ...
							        if(response.status)
            						{window.location.href="<?php echo base_url().'admin/sale/'; ?>";}
 ...
 obj.url="<?php echo base_url();?>";
	...
	$.ajax({
        ...
        data:{
        	id: obj.id,
        	source_module:"sale",
        	module:"<?php echo encode_id('sale'); ?>",
...
<!-- STAMP and Cancel-->
<script>
$(document).on("click","a#do_sat",function(){

var method="";
if($(this).data("sat")=="stamp")
method="stampXmlPac";

if($(this).data("sat")=="cancel")
method="xmlToCancel";

		$.ajax({
	        type: "POST",
	        url: "<?php echo base_url().'xml/';?>"+method,
	        dataType:"json",
	        async:true,
	        data:{  
...
var client_email=$("input#client_email").val();
	...
	$.ajax({
		...
		// ...
			...
			// traer la informacion de los correos 
			get_emails("<?php echo encode_id('sale/');?>","<?php echo encode_id($id);?>");	        		
        }
