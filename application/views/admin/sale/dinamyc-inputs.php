...
$form["status"]=form_dropdown('status',$sys["forms_fields"]["sales_status"],$status,"id='status'");
...
<?php  echo form_close();?>
	                        ...
	                        <div class="form-group">
	                        	...
	                        	<div class="area3 containerButtons buttonsActions">
									...
									<a class="button" href="<?php echo base_url().'payment/admin/?source_module='.encode_id($module_data["link"]."payment/").'&id='.encode_id($id).'&module='.encode_id("sale");?>">
										ver pagos
	                        	...
	                        	<!-- factura electronica habilitada -->
	                        	<?php if(!empty($sys["config"]["invoice_electronic_enabled"])):?>
		                        	<div class="area3 containerButtons buttonsActions">
									<?php if(!empty($id)){ ?>
										
										<a class="button <?php echo (!empty($sat_stamp)?'':'buttonNotFile'); ?>" href="<?php echo $sat_stamp? base_url().'sale/download_file/?name_file='.encode_id($folio.'.xml').'&file_path='.encode_id($sat_stamp):'javascript:void(0);' ?>" <?php echo file_exists($sat_stamp)?"":"id='do_sat'; data-sat='stamp';"; ?>>
										<?php $text=($sat_stamp?"Descargar":"Generar"); ?>
										<?php echo $text;?>,CFDI
										<span class='certificate'></span>
										</a>
	  
										<?php if($status==3): //cancelada  ?>
										<a class="button <?php echo (!empty($sat_cancel)?'':'buttonNotFile') ?>" href="<?php echo $sat_cancel? base_url().'sale/download_file/?name_file='.encode_id($folio.'-C.xml').'&file_path='.encode_id($sat_cancel):'javascript:void(0);' ?>" <?php echo file_exists($sat_cancel)?"":"id='do_sat' data-sat='cancel'"; ?>>
										<?php $text=($sat_cancel?"Descargar":"Generar"); ?>
											<?php echo $text;?> CFDI,Cancelar
										<span class='certificate'></span>
										</a>
										<?php endif; ?>
									<?php } ?>
							...
							<div class="articleListContainer documentViewDefault col-sm-12 col-md-12 col-lg-12">
								...
								<div class="area2">
									<div class="header">
										...
										<div class="price">P.U.</div>
										<div class="totalSub">Sub.</div>
