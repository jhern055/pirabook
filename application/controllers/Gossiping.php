...
class Gossiping extends CI_Controller {
	public  $config;
	public  $page;
	public 	$registred_by;
	public 	$idTemp;
	...
	$this->load->model("publication/gossiping/gossiping_model");
    ...
    $uri_string=(!empty($uri_string["uri_string"])? $uri_string["uri_string"]:$this->uri->uri_string );
		...
		// Remissiones <gossiping>		
		$this->uri_gossiping="gossiping/";
		// </gossiping>	
	...
	// </limpiar>
		...
		if($method=="gossipingView"){ 

    	// no vacios
    		$response_required=$this->required_fields("gossiping",$http_params);
		...
		if($this->gossiping_model->record_same_gossiping($data,$id) )
		return array("status"=>0,"msg"=>"Ya existe un registro identico ","data"=>false);

		$data=array_merge($data_depend,$data);

		if($id)
		$last_id=$this->gossiping_model->update_gossiping($data,$id);
		else{
		$last_id=$this->gossiping_model->insert_gossiping($data);
					// <own>
					...
					// mover y insertar en el registro
						...
						// print_r(json_encode($tmp_move));
							foreach ($tmp_move as $tmp_mv_name) {
								...
								$response=move_file("gossiping_file",$tmp_mv_name["filename"]);
							...
							// </mover>
						$this->file_model->insert_file_uni("gossiping",$data_tmp);
					endif;
					// </own>			
		}
...
// <gossiping> 
	...
	// carga ajax
	public function gossiping_ajax() {

	$module=$this->uri_gossiping;

	$http_params=array_merge($_GET,$_POST);
	$http_params          =array(
	"input_search_gossiping" =>(!empty($http_params["input_search_gossiping"]) ? strip_tags( $this->security->xss_clean( $http_params["input_search_gossiping"] ) ) :""),
	"show_amount_gossiping"  =>(!empty($http_params["show_amount_gossiping"]) ? strip_tags( $this->security->xss_clean( $http_params["show_amount_gossiping"])  ) :20),
	);
	extract($http_params);

	$page_amount=$show_amount_gossiping;

	$query_search=array();
	if(!empty($input_search_gossiping)){
	$query_search=array(
		"\$this->pr->like('title', \"".$input_search_gossiping."\");",
		);
	}

	$this->pagination->initialize($this->config_pagination("gossiping/gossiping_ajax",$this->gossiping_model->get_gossiping_amount($query_search),$page_amount) );

	$data=array(
		"module"=>$module,
		"sys"=>$this->sys,
		"input_search_gossiping"=>$input_search_gossiping,
		"show_amount"=>$show_amount_gossiping,
		"records_array"=>$this->gossiping_model->get_gossiping($page_amount, $this->page,$query_search),
		"pagination"=>$this->pagination->create_links(),
		"module_data"=>$this->gossiping_model->m_name($module),
		);
	$data["module_data"]["module_data_method_do_it"]="gossiping/gossipingView/";

    $data["modules_quick"]=$this->load->get_back_access($module);

	// sale/gossiping/
	$html=$this->load->view("gossiping/ajax/table-gossiping-view",$data,true);
	$data["html"]=$html;

	// $this->session->set_userdata('record_start_row_gossiping',$this->page);
	...
	// $data=$this->gossiping_ajax();

	// $this->session->set_userdata("idTemp");
	// $this->session->set_userdata('input_search_gossiping');
	// $this->session->set_userdata("sessionMode_gossiping");

	// // if(!empty($this->page) and !empty($data["records_array"]))
	// // $this->session->set_userdata('record_start_row_gossiping',$this->page);

	// if(!empty($_POST["ajax"]))
	// return print_r(json_encode(array("status"=>1,"msg"=>"HtmlConExito","html"=>$this->load->view("gossiping/gossiping_view",$data,true) ))) ;
	// else
	// return $this->load->template("gossiping/gossiping_view",$data);
	redirect("gossiping/gossipingView");
	}	

	public function gossipingView($id=null) {
	...
	$return=$this->do_it($id,"gossipingView");
	...
	// traer el registro 
	$array['data']=$this->gossiping_model->get_gossiping_id($id);
	...
	// <F5>
	$sessionMode=$this->session->userdata("sessionMode_gossiping");
	if(!empty($_POST)):
		$this->session->set_userdata("sessionMode_gossiping",$array['data']['MODE']);
	endif;
	$array['id']=$id;
	// </F5>
	...
	// nombre del modulo
	// <OWN>
	$array['data']["module_data"]=$this->gossiping_model->m_name($module);	
	$array['data']["module_data_method_do_it"]="gossiping/gossipingView/";
	...
	public function gossiping_delete() {
    ...
    // borrar los pagos 
    $this->link_model->delete_links("gossiping",$id,null);

    // <imagenes> borrar las imagenes 
    $imagenes=$this->gossiping_model->get_gossiping_images($id);
    if(!empty($imagenes))
    foreach ($imagenes as $ĸ => $row)
    @unlink(FCPATH.$this->sys["storage"]["gossiping_config"].$row["filename"]);

    $this->gossiping_model->delete_all_image_gossiping($id);
    // </imagenes>
	if($this->gossiping_model->gossiping_delete_it($id))
