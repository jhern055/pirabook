...
class Quatition extends CI_Controller {
	public  $config;
	public  $page;
	public 	$registred_by;
	public 	$idTemp;
	...
	$this->load->model("admin/sale/quatition/quatition_model");
    ...
    $uri_string=(!empty($uri_string["uri_string"])? $uri_string["uri_string"]:$this->uri->uri_string );
		...
		// Remissiones <quatition>		
		$this->uri_quatition="admin/sale/quatition/";
		// </quatition>	
	...
	"exchange_rate"    =>(!empty($http_params["exchange_rate"]) ?strip_tags( $this->security->xss_clean($http_params["exchange_rate"]) ):""),
		...
		if($this->quatition_model->record_same_quatition($data,$id) )
		return array("status"=>0,"msg"=>"Ya existe un registro identico ","data"=>false);

		$data=array_merge($data_depend,$data);

		if($id)
		$last_id=$this->quatition_model->update_quatition($data,$id);
		else
		$last_id=$this->quatition_model->insert_quatition($data);

		// processar details
		$response_detail=$this->processDetail("quatition",$last_id);
...
// <quatition> 
	...
	// carga ajax
	public function quatition_ajax() {

	$module=$this->uri_quatition;

	$http_params=array_merge($_GET,$_POST);
	$http_params          =array(
	"input_search_quatition" =>(!empty($http_params["input_search_quatition"]) ? strip_tags( $this->security->xss_clean( $http_params["input_search_quatition"] ) ) :""),
	"show_amount_quatition"  =>(!empty($http_params["show_amount_quatition"]) ? strip_tags( $this->security->xss_clean( $http_params["show_amount_quatition"])  ) :10),
	);
	extract($http_params);

	$page_amount=$show_amount_quatition;

	$query_search=array(
		"\$this->db->like('folio', \"".$input_search_quatition."\");",
		);

	$this->pagination->initialize($this->config_pagination("admin/sale/quatition/quatition_ajax",$this->quatition_model->get_quatition_amount($query_search),$page_amount) );

	$data=array(
		"module"=>$module,
		"sys"=>$this->sys,
		"input_search_quatition"=>$input_search_quatition,
		"show_amount"=>$show_amount_quatition,
		"records_array"=>$this->quatition_model->get_quatition($page_amount, $this->page,$query_search),
		"pagination"=>$this->pagination->create_links(),
		"module_data"=>$this->quatition_model->m_name($module),
		);
	$data["module_data"]["module_data_method_do_it"]="admin/sale/quatition/quatitionView/";

    $data["modules_quick"]=$this->load->get_back_access($module);

	// sale/quatition/
	$html=$this->load->view("admin/sale/quatition/ajax/table-quatition-view",$data,true);
	$data["html"]=$html;

	// $this->session->set_userdata('record_start_row_quatition',$this->page);
	...
	$data=$this->quatition_ajax();

	$this->session->set_userdata("idTemp");
	$this->session->set_userdata('input_search_quatition');
	$this->session->set_userdata("sessionMode_quatition");

	// if(!empty($this->page) and !empty($data["records_array"]))
	// $this->session->set_userdata('record_start_row_quatition',$this->page);

	if(!empty($_POST["ajax"]))
	return print_r(json_encode(array("status"=>1,"msg"=>"HtmlConExito","html"=>$this->load->view("admin/sale/quatition/quatition_view",$data,true) ))) ;
	else
	return $this->load->template("admin/sale/quatition/quatition_view",$data);

	}

	public function quatitionView($id=null) {
	...
	$data_before=$this->quatition_model->get_quatition_id($id);
	...
	$return=$this->do_it($id,"quatitionView");
	...
	// traer el registro 
	$array['data']=$this->quatition_model->get_quatition_id($id);
	...
	// <F5>
	$sessionMode=$this->session->userdata("sessionMode_quatition");
	if(!empty($_POST)):
		$this->session->set_userdata("sessionMode_quatition",$array['data']['MODE']);
	endif;
	$array['id']=$id;
	// </F5>

	// nombre del modulo
	$array['data']["module_data"]=$this->quatition_model->m_name($module);	
	$array['data']["module_data_method_do_it"]="admin/sale/quatition/quatitionView/";
	...
	public function quatition_delete() {
    ...
    // borrar los detalles 
    $this->article_model->delete_details("quatition",$id,null);

	if($this->quatition_model->quatition_delete_it($id))
