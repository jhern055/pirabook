...
class Remission extends CI_Controller {
	public  $config;
	public  $page;
	public 	$registred_by;
	public 	$idTemp;
	...
	"exchange_rate"    =>(!empty($http_params["exchange_rate"]) ?strip_tags( $this->security->xss_clean($http_params["exchange_rate"]) ):""),
		...
		if($this->remission_model->record_same_remission($data,$id) )
		return array("status"=>0,"msg"=>"Ya existe un registro identico ","data"=>false);

		$data=array_merge($data_depend,$data);

		if($id)
		$last_id=$this->remission_model->update_remission($data,$id);
		else
		$last_id=$this->remission_model->insert_remission($data);

		// processar details
		$response_detail=$this->processDetail("remission",$last_id);
...
// <remission> 
	...
	// carga ajax
	public function remission_ajax() {

	$module=$this->uri_remission;

	$http_params=array_merge($_GET,$_POST);
	$http_params          =array(
	"input_search_remission" =>(!empty($http_params["input_search_remission"]) ? strip_tags( $this->security->xss_clean( $http_params["input_search_remission"] ) ) :""),
	"show_amount_remission"  =>(!empty($http_params["show_amount_remission"]) ? strip_tags( $this->security->xss_clean( $http_params["show_amount_remission"])  ) :10),
	);
	extract($http_params);

	$page_amount=$show_amount_remission;

	$query_search=array(
		"\$this->db->like('folio', \"".$input_search_remission."\");",
		);

	$this->pagination->initialize($this->config_pagination("admin/sale/remission/remission_ajax",$this->remission_model->get_remission_amount($query_search),$page_amount) );

	$data=array(
		"module"=>$module,
		"sys"=>$this->sys,
		"input_search_remission"=>$input_search_remission,
		"show_amount"=>$show_amount_remission,
		"records_array"=>$this->remission_model->get_remission($page_amount, $this->page,$query_search),
		"pagination"=>$this->pagination->create_links(),
		"module_data"=>$this->remission_model->m_name($module),
		);
	$data["module_data"]["module_data_method_do_it"]="admin/sale/remission/remissionView/";

    $data["modules_quick"]=$this->load->get_back_access($module);

	// sale/remission/
	$html=$this->load->view("admin/sale/remission/ajax/table-remission-view",$data,true);
	$data["html"]=$html;

	// $this->session->set_userdata('record_start_row_remission',$this->page);
	...
	$data=$this->remission_ajax();

	$this->session->set_userdata("idTemp");
	$this->session->set_userdata('input_search_remission');
	$this->session->set_userdata("sessionMode_remission");

	// if(!empty($this->page) and !empty($data["records_array"]))
	// $this->session->set_userdata('record_start_row_remission',$this->page);

	if(!empty($_POST["ajax"]))
	return print_r(json_encode(array("status"=>1,"msg"=>"HtmlConExito","html"=>$this->load->view("admin/sale/remission/remission_view",$data,true) ))) ;
	else
	return $this->load->template("admin/sale/remission/remission_view",$data);

	}

	public function remissionView($id=null) {
	...
	$data_before=$this->remission_model->get_remission_id($id);
	...
	$return=$this->do_it($id,"remissionView");
	...
	// traer el registro 
	$array['data']=$this->remission_model->get_remission_id($id);
	...
	// <F5>
	$sessionMode=$this->session->userdata("sessionMode_remission");
	if(!empty($_POST)):
		$this->session->set_userdata("sessionMode_remission",$array['data']['MODE']);
	endif;
	$array['id']=$id;
	// </F5>

	// nombre del modulo
	$array['data']["module_data"]=$this->remission_model->m_name($module);	
	$array['data']["module_data_method_do_it"]="admin/sale/remission/remissionView/";
	...
	public function remission_delete() {
    ...
    // borrar los detalles 
    $this->article_model->delete_details("remission",$id,null);

    // borrar los pagos 
    $this->payment_model->delete_payments("remission",$id,null);

	if($this->remission_model->remission_delete_it($id))
