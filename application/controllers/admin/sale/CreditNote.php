...
class CreditNote extends CI_Controller {
	...
	$this->load->model("admin/sale/creditNote/creditNote_model");
    ...
    $uri_string=(!empty($uri_string["uri_string"])? $uri_string["uri_string"]:$this->uri->uri_string );
		...
		// Remissiones <creditNote>		
		$this->uri_creditNote="admin/sale/creditNote/";
		// </creditNote>	
	...
	"exchange_rate"    =>(!empty($http_params["exchange_rate"]) ?strip_tags( $this->security->xss_clean($http_params["exchange_rate"]) ):""),
		...
		if($this->creditNote_model->record_same_creditNote($data,$id) )
		return array("status"=>0,"msg"=>"Ya existe un registro identico ","data"=>false);

		$data=array_merge($data_depend,$data);

		if($id)
		$last_id=$this->creditNote_model->update_creditNote($data,$id);
		else
		$last_id=$this->creditNote_model->insert_creditNote($data);

		// processar details
		$response_detail=$this->processDetail("credit_note",$last_id);
...
// <creditNote> 
	...
	// carga ajax
	public function creditNote_ajax() {

	$module=$this->uri_creditNote;

	$http_params=array_merge($_GET,$_POST);
	$http_params          =array(
	"input_search_creditNote" =>(!empty($http_params["input_search_creditNote"]) ? strip_tags( $this->security->xss_clean( $http_params["input_search_creditNote"] ) ) :""),
	"show_amount_creditNote"  =>(!empty($http_params["show_amount_creditNote"]) ? strip_tags( $this->security->xss_clean( $http_params["show_amount_creditNote"])  ) :10),
	);
	extract($http_params);

	$page_amount=$show_amount_creditNote;

	$query_search=array(
		"\$this->db->like('folio', \"".$input_search_creditNote."\");",
		);

	$this->pagination->initialize($this->config_pagination("admin/sale/creditNote/creditNote_ajax",$this->creditNote_model->get_creditNote_amount($query_search),$page_amount) );

	$data=array(
		"module"=>$module,
		"sys"=>$this->sys,
		"input_search_creditNote"=>$input_search_creditNote,
		"show_amount"=>$show_amount_creditNote,
		"records_array"=>$this->creditNote_model->get_creditNote($page_amount, $this->page,$query_search),
		"pagination"=>$this->pagination->create_links(),
		"module_data"=>$this->creditNote_model->m_name($module),
		);
	$data["module_data"]["module_data_method_do_it"]="admin/sale/creditNote/creditNoteView/";

    $data["modules_quick"]=$this->load->get_back_access($module);

	// sale/creditNote/
	$html=$this->load->view("admin/sale/creditNote/ajax/table-creditNote-view",$data,true);
	$data["html"]=$html;

	// $this->session->set_userdata('record_start_row_creditNote',$this->page);
	...
	$data=$this->creditNote_ajax();

	$this->session->set_userdata("idTemp");
	$this->session->set_userdata('input_search_creditNote');
	$this->session->set_userdata("sessionMode_creditNote");

	// if(!empty($this->page) and !empty($data["records_array"]))
	// $this->session->set_userdata('record_start_row_creditNote',$this->page);

	if(!empty($_POST["ajax"]))
	return print_r(json_encode(array("status"=>1,"msg"=>"HtmlConExito","html"=>$this->load->view("admin/sale/creditNote/creditNote_view",$data,true) ))) ;
	else
	return $this->load->template("admin/sale/creditNote/creditNote_view",$data);

	}

	public function creditNoteView($id=null) {
	...
	$data_before=$this->creditNote_model->get_creditNote_id($id);
	...
	$return=$this->do_it($id,"creditNoteView");
	...
	// traer el registro 
	$array['data']=$this->creditNote_model->get_creditNote_id($id);
	...
	// <F5>
	$sessionMode=$this->session->userdata("sessionMode_creditNote");
	if(!empty($_POST)):
		$this->session->set_userdata("sessionMode_creditNote",$array['data']['MODE']);
	endif;
	$array['id']=$id;
	// </F5>

	// nombre del modulo
	$array['data']["module_data"]=$this->creditNote_model->m_name($module);	
	$array['data']["module_data_method_do_it"]="admin/sale/creditNote/creditNoteView/";
	...
	public function creditNote_delete() {
    ...
    // borrar los detalles 
    $this->article_model->delete_details("credit_note",$id,null);

	if($this->creditNote_model->creditNote_delete_it($id))
