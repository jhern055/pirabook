...
class Publication extends CI_Controller {
	...
	// </limpiar>
		...
		$last_id=$this->publication_model->insert_publication($data);
			...
			// return print_r(json_encode(array($file_name)));
					...
					// mover y insertar en el registro
						...
						// print_r(json_encode($tmp_move));
							foreach ($tmp_move as $tmp_mv_name) {
								...
								$response=move_file("publication_file",$tmp_mv_name["filename"],$last_id);
...
// <publication> 
	...
	"input_search_rar_name" =>(!empty($http_params["input_search_rar_name"]) ? strip_tags( $this->security->xss_clean( $http_params["input_search_rar_name"] ) ) :""),
	"input_search_id" =>(!empty($http_params["input_search_id"]) ? strip_tags( $this->security->xss_clean( $http_params["input_search_id"] ) ) :""),
	"publication_status" =>(!empty($http_params["publication_status"]) ? strip_tags( $this->security->xss_clean( $http_params["publication_status"] ) ) :""),
	...
	$query_search=array(
		"\$this->db->like('title', \"".$input_search_publication."\");",
		);
	}

	if(!empty($input_search_rar_name)){
	$query_search=array_merge($query_search,array(
		"\$this->db->like('rar_name', \"".$input_search_rar_name."\");",
		)
	);
	}

	if(!empty($input_search_id)){
	$query_search=array_merge($query_search,array(
		"\$this->db->where('id', \"".$input_search_id."\");",
		)
	);
	}
	...
	$query_search=array_merge($query_search,array(
		"\$this->db->where('status', \"".$publication_status."\");",
		)
	);
	// }
		$query_search=array_merge($query_search,array("\$this->db->order_by('title', 'asc');") );
	...
	$data=array(
		...
		"input_search_rar_name"    =>$input_search_rar_name,
    ...
    // borrar grupos 
    $this->link_model->delete_groups_link($id,null);
    ...
    @unlink(FCPATH.$this->sys["storage"]["publication_config"].$id_implode."/".$row["filename"]);
